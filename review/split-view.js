...
export function renderSplitView({
  ...
  const railItems = list
    .map((c, i) => {
      ...
      return `
        <button type="button" class="rv-rail-item rv-rail-item-site${i === focusIndex ? " is-on" : ""}" data-index="${i}">
          ${favSlotHtml(host)}
  ...
  const host = hostnameOf(focus);
  const hostLabel = host || "未记录来源";
